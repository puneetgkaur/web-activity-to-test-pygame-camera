define(["webL10n",
    ...
    activity.init_camera = function (callback) {
        function onResponseReceived(error, result) {
            if (error === null) {
                callback(null, result[0]);
            } else {
        ...
        bus.sendMessage("activity.init_camera", [], onResponseReceived);	
    }

    activity.get_cam_image = function (callback) {
        function onResponseReceived(result, binary) {
                callback(result, binary);
        }

        bus.sendMessage("activity.get_cam_image", [], onResponseReceived);	
    }

    activity.stop_camera = function (callback) {
        function onResponseReceived(error, result) {
            if (error === null) {
                callback(null, result[0]);
            } else {
        ...
        bus.sendMessage("activity.stop_camera", [], onResponseReceived);	
    }
